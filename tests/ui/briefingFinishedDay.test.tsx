...
describe('Briefing — the finished-day guard (gr6-008)', () => {
  ...
  it('prefers whichever record the day actually holds when only prereg was ever available', async () => {
    // W12 will unlock Prereg Mode on the first COMPLETED day rather than the
    // first retraction. This asserts the property that change must preserve:
    // the guard is a function of which modes are still PLAYABLE, so widening
    // "unlocked" cannot make a finished day playable again.
    const preregRec = record({ mode: 'prereg', shareString: '🧾📄 prereg day' });
    seed(
      freshV1({
        history: { [ISO]: { hack: record(), prereg: preregRec } },
        achievements: { first_retraction: '2026-08-01' },
      })
    );
    renderBriefing();
