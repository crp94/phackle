...
- **Determinism:** same date ⇒ byte-identical puzzle on all clients **running the same JS engine** (what the golden fixtures pin). Engine uses only `+ − × ÷ sqrt exp log` on f64, `Math.imul` and the spec-exact integer ops; **never** `Math.pow` or `**`, never trig, `hypot`/`cbrt`/`expm1`/`log1p`/`log2`/`log10`/`fround`, `Intl.*` or `toLocale*`, never `Math.random` (§3.1) — all ESLint-enforced from W5 (gr6-095), not prose. ACROSS engines the guarantee is weaker and must not be overstated: `Math.exp`/`Math.log` are implementation-approximated per ECMA-262 and may differ by ≤1 ULP between V8, SpiderMonkey and JavaScriptCore, so cross-engine agreement is established EMPIRICALLY by the E2E browser matrix, never asserted (gr6-048). See `src/engine/prng.ts`'s header for the full statement.
- **Engine purity:** `src/engine/**` imports nothing from `react`, `src/ui`, `src/game`, `src/i18n`, `src/content` (ESLint-enforced from T1), with exactly TWO sanctioned exceptions: `src/game/tuning.ts` (pure constants, no side effects — keeps §3.9's single-tuning-file rule compatible with purity) and the checksum-guarded lookup tables in `src/data/*.json` (language-blind build artefacts, asserted against the live DGP constants at every `init`). The second was corrected in W5 (gr6-095): `reveal.ts` had imported `src/data/p_hit_by_k.json` since T10 while this sentence and the lint rule both said "the sole allowed exception is tuning.ts" — the stated rule was false, and no pattern covered the import. Engine is language-blind.
...
| 1a | T2 stats · T3 dgp · T4 i18n+contracts · T28 design foundation (parallel, disjoint files) | T5 gates on T4+T28 merge; T6 on T4 |
| 1b | T5 shell+theme · T6 EN content (parallel; T2/T3 may still run) | T7 needs T2+T3 only |
...
| 4 | T19 IT · T20 ES (after copy freeze) · T21 PWA+OG · T29 visual polish (after T18, before T22) · T22 a11y pass · T23 E2E · T24 CI finalize → T25 deploy → T26 QA+launch → T27 website link (optional) | |
...
- [ ] **Goldens**: `scripts/gen_goldens.ts` (tsx) runs 5 fixed dates `2026-09-01, 2026-10-31, 2026-12-25, 2027-01-01, 2027-07-04` → writes `{isoDate, dayType, scenarioIndexFor20, attemptUsed, rows40Hash, curve200Hash, sigCount200}` (NO puzzleNumber — it is EPOCH-derived and would need re-hashing for a field this suite is not about). Commit fixtures + test that regenerating in-process matches committed bytes. This guards drift; correctness came from T2/T3/T7 fixtures. **Corrected (gr6-051):** an earlier version of this line said "EPOCH changes at T25 must not break goldens". That is false. Excluding `puzzleNumber` removes one EPOCH-derived input, not the other: `seeds.ts`'s `scenarioIndexFor` gates the 14-day no-repeat exclusion walk on `iso > EPOCH`, and that walk recurses back through every date after EPOCH — so moving EPOCH re-indexes the chain and changes the scenario (and therefore every hash) for dates it has not even reached. Measured against today's fixtures: unchanged for EPOCH ≤ 2026-08-23, four of the five break from EPOCH ≥ 2026-08-24 — eight days before the earliest golden date. Moving EPOCH is therefore a **regenerate-and-commit-the-goldens** deploy step, enforced by `tests/game/epochGuard.test.ts`.
...
## Owner feedback round 5 (2026-08-04, post-trilingual-merge) — Tasks T33-T35

Owner directives, verbatim intent: (1) default the interface to English; (2) the language menu should have a little flag; (3) the dark/paper selection should be clearer; (4) a Home/Play button — hard to get back to the game from the menus; (5) two-emoji glyphs (e.g. the green check) must not overlap text; (6) the UX/UI should be more animated, reads plain.

### Task T33: Navigation & controls clarity (opus; after T29 merge)

- [ ] **Locale default:** `detectLocale` returns the stored choice if present, else `'en'` — browser-language auto-detection removed by owner directive (delta-spec §2 superseded). Amend its tests; keep the prefix-match code path deleted, not dead.
- [ ] **Flags in the language menu:** each locale button gets its flag glyph (🇬🇧 en / 🇮🇹 it / 🇪🇸 es) beside the code text — text stays (flags alone fail on Windows Chrome, which renders letter pairs); aria-labels stay language names, not flags.
- [ ] **Theme toggle clarity:** the paper/dark control must state what it does and which state is active (both label and pressed-state affordance; DESIGN.md-compliant, no new colors).
- [ ] **Home/Play:** the masthead title becomes a link back to the game, AND an explicit nav affordance (Play/Home) appears wherever Legend/About/Stats render — whichever screen you're on, one obvious tap returns to today's puzzle. Do not disturb in-day state (returning mid-day resumes the day's screen from the store, never restarts).
- [ ] **finishReveal wiring (escalated blocker折):** end-of-Reveal button calling `store.finishReveal()` → Summary reachable. New copy key (EN + IT + ES, transcreated register, ≤1 em dash, no tokens) + tests incl. a full-flow reveal→summary DOM test.
- [ ] **legend.emojiSpec IT/ES mirrors:** transcreate T29's new EN value (six-knob enumeration) into both locales.
- [ ] Gate + spoiler/scan suites untouched-or-stronger; 2px masthead overflow (ph-header__controls, GR4a note) may be fixed here if trivially adjacent.

### Task T34: Compound-emoji rendering audit (sonnet; after T29 merge, parallel with T33 — file-disjoint: T34 owns Legend/ForkTrail/Summary emoji markup+CSS, T33 owns shell/nav/copy)

- [ ] Audit EVERY site rendering multi-emoji sequences (⚖️✅, ⚖️❌, trail strings, legend rows, share preview in Summary, achievements): screenshot at 360/768/1088 both themes, find any glyph-on-text or glyph-on-glyph overlap (the ✅-overprinting-text class Carlos saw).
- [ ] Fix via spacing that survives font fallback (explicit gap/inline-block/letter-spacing on a wrapping span — no reliance on emoji font metrics; variation selector-16 where a glyph renders as text-style). Evidence: before/after shots per fixed site.

### Task T35: Motion design pass (opus, aesthetics agent; AFTER T33+T34 merge — motion choreographs the final layout)

- [ ] Owner directive supersedes the frozen motion budget: DESIGN.md's motion law is AMENDED (not ignored) — a registered motion system replaces the exhaustive 4-item list. Every animation still: purposeful (enters meaning, not decoration), token-timed (shared duration/easing scale), and with `prefers-reduced-motion` parity (reduced ≠ broken — content appears without motion).
- [ ] Candidate choreography (agent judges, owner taste = Nothing-OS restraint): screen transitions (crossfade/slide of the router), dial sweep on result, stamp impact, press clippings staggered entrance, reveal's sequential Block cadence, popover fade/scale, achievement unlock moment, share-copied toast. The game should feel alive at the moments that matter, not busy everywhere.
- [ ] tokens.test.ts motion enforcement updated to enforce the NEW registry (exhaustive again after amendment); reduced-motion E2E note handed to T23.

### Task T39: Game-dependent press (owner directive, round 7)

Owner: "Can we do the 'simulated press' page a bit more game-dependent? so at least some of the news are related to the research question of the game."

- [ ] **T39a (EN + mechanism, opus):** every one of the 20 scenarios gets ≥1 bespoke press item (tier chosen per scenario by comedic fit — the picker's tier mix stays 1 prestige / 1 midmarket / 1 chyron per day where the bank allows); `pickPress` guarantees the scenario-bound item appears on its scenario's day, generic items fill the rest; SPOILER LAW: bespoke items riff on the QUESTION/method/cover-story furniture, never the outcome or verdict (Published renders on both day types — a blurb that implies significance or nullity leaks the reveal; mechanically testable: bespoke blurbs contain no day-type-correlated claims, and the existing press-shape tests extend to the new rows). Register: tier voice laws unchanged; {effect} token rules if a headline-form item uses it. Deterministic: same day → same press, seeded as today.
- [ ] **T39b (IT+ES transcreation, opus):** the new bespoke items transcreated per the delta rules (outlets transcreated, register per tier, convention contracts); per-locale comedy review as with the corpus.
