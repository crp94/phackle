...
- **Engine purity:** `src/engine/**` imports nothing from `react`, `src/ui`, `src/game`, `src/i18n`, `src/content` (ESLint-enforced from T1), with exactly ONE sanctioned exception: `src/game/tuning.ts` (pure constants, no side effects — keeps §3.9's single-tuning-file rule compatible with purity). Engine is language-blind.
