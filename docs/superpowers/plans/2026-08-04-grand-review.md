...
## Amendment A4 (2026-08-04, owner: "no rush, I prefer it to be perfect")

- **GR5/GR6 triage rule superseded:** `polish` findings are FIXED pre-deploy like `blocker`/`high` — parked-with-ruling is reserved for wontfix-with-reasoning only. Batch by file locality as before; there is no deadline pressure trading against completeness.
- **Explicit GR2/GR3 input question:** does the press mix (1 scenario-bound + 2 generic items/day) read as a seam? If either lane flags it, the 60-cell bespoke matrix (T39c: 34 EN items + transcreations) becomes a fix task, not a parked follow-up.
- **All previously parked taste/deferred items route through GR lanes for a verdict, none auto-expire:** dial-settle 2px restraint (GR4), header 3-row height at 360 (GR4), dual Legend-named buttons (GR4), nested dialog on published path (GR4/GR1b), masthead watermark head-vs-foot (GR4), practice-mode "Vol. 1, No. -5" (GR1b/GR2), 320w residual overflow (GR4), pressForDay() refactor + press reachability test (GR1b), validator-extraction from shape.test (GR1c).
- Deploy timing is owner's-morning-whenever — the pipeline optimizes for completeness, not the clock.
